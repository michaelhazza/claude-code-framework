...
### The mockups in `_archive/prototypes/cached-context/` (origin-repo example — directory not shipped with the framework)

The five mockups in the origin repo's `_archive/prototypes/cached-context/` were generated before this doc existed. They violate rules 1, 2, 3, 4 — most of them represent "what the backend could surface if we exposed every column", not "what the user needs to complete the task".
