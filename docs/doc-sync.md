...
| `docs/design-language.md` (if present; scaffolded from `docs/design-language-template.md`) | Any change to tokens, type system, component recipes, shell, or the craft bar — including learnings from a mockup polish round that altered the house style. Repos without this doc record `n/a — not present in this repo`. |
| `docs/mobile-capability-principles.md` / `docs/behaviour-manifest-template.md` | Any change to the mockup pipeline's grounding or interaction-behaviour contract (capture manifest shape, behaviour-checklist rows, the `mockup-designer` / `mockup-reviewer` / `mockup-coordinator` / `spec-coordinator` prose that references them). Keep the docs and the agent prose in lockstep. |
...
| `references/skill-overlay-convention.md` | When the skill-overlay mechanism changes: the canonical pointer-line wording, the `skill-context.md` overlay shape, the KNOWLEDGE→overlay→canonical drain protocol, the `/cleanfiles` drain target, the `validate-framework.js` pointer gate, or framework-doctor Checks 6/7. Keep the doc, the pointer line in all `SKILL.md`, and the validator in lockstep. |
| `references/eval-suite-format.md` | When the eval-suite contract changes: `config.json`/`cases.jsonl`/`baseline.json` shape, the `expected.verdict` taxonomy, the normalizer contract, the catchRate/falseAlarmRate metric definitions, or the runner's run/accept/dry-run/missing-baseline semantics. Keep this doc, `scripts/eval-prompts*.ts`, the `/eval-prompts` command, and framework-doctor Check 8 in lockstep. |
