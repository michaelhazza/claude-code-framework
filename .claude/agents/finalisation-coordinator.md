...
**Read the signal first:** if `tasks/review-logs/metrics/` contains harness-metrics reports (generated by `scripts/harness-metrics.ts`), read the latest one before drafting proposals — proposals grounded in a measured trend (rising FP-proxy for a reviewer, climbing fix-loop iterations, falling auto-apply success) outrank pattern hunches, and a metric moving the wrong way is itself a proposal trigger. No reports → proceed as before and note "no metrics available" in the proposal table header.
