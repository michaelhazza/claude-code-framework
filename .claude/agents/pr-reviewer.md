...
2. `architecture.md` — all patterns, conventions, and constraints that must be enforced. Read if present; skip when the repo has not authored one. **Pack-sliced load (preferred):** if `docs/context-packs/review.md` exists and contains no `{{ARCHITECTURE_ANCHOR` placeholder tokens, load only the `architecture.md` sections named in its `## Sources` block (anchor-slice mechanics per `.claude/agents/context-pack-loader.md` Step 2) and honour its `## Skip` conditionals — loading a skipped section when the changed files match its condition (e.g. LLM routing rules when the diff touches LLM code). If any named anchor fails to resolve, fall back to the whole-file read. Note which mode you used in the review log header: `context-load: review pack (<N> sections)` or `context-load: full architecture.md (<reason>)`.
