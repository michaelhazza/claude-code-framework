...
- **Capability-check failure states drawn.** If the screen consumes a capability check (push permission, biometric login, secure token storage, native file picker, payment API, geolocation, microphone/camera, WebAuthn — anything that can return granted / denied / unsupported / wrapper-required / transport-failed), draw the failure-state UI for at least one denied/unsupported reason — not just the granted+success path. The deferred-by-default rule does NOT apply to capability failure states; they are the half of the design that prevents silent-authorisation bugs.
...
- **Tier classification declared per screen** (mobile-touching mockups only). Declare in the round summary which tier of mobile polish the screen targets:
  ...
  - If you cannot pick, ask the caller before drawing. Inconsistent tier choices across a multi-screen prototype are a rework finding.
