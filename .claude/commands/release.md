...
7. **Re-run the version-consistency check** from step 3 against the bumped files (including the new CHANGELOG heading). **Then run the growth gate** — `node scripts/gates/verify-growth-gate.mjs` — which diffs new behavioural files since the previous release tag and fails (exit 1) if any lacks its `> growth-gate:` CHANGELOG declaration (control C5). Fix the CHANGELOG entry before proceeding; the gate fails-open only when the previous tag is unresolvable. Then commit:
