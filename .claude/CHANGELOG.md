...
**Fixed:**
- `.claude/agents/spec-reviewer.md § Step 1` — replaced `codex review --file/--rubric` (+ `--stdin` fallback), which are not valid flags in current Codex CLI, with `codex exec -s read-only --skip-git-repo-check "<review prompt>" < "${SPEC_PATH}"` plus a bare-`codex exec` retry for older Codex without the `-s` / `--skip-git-repo-check` flags. The Codex spec-review tier now reviews the spec document instead of erroring on unknown arguments.
- `scripts/chatgpt-review.ts` — optionally load `dotenv/config` via `createRequire(import.meta.url)` wrapped in try/catch, so `OPENAI_API_KEY` can live in a local `.env`; repos without the `dotenv` package are unaffected (the import is a no-op).
