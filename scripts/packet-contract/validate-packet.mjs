 ...
 *
 * `validatePacket` never throws — it returns `{ok, errors[]}` so a caller can
 * report a malformed packet without a try/catch at every call site.
 */
...
export async function validatePacket(kind, obj) {
  if (kind !== 'work' && kind !== 'completion') {
    return { ok: false, errors: [`unknown packet kind: ${kind}`] };
  }

  const validate = await getSchemaValidator(kind);
  if (validate) {
    if (validate(obj)) return { ok: true, errors: [] };
    const errors = (validate.errors ?? []).map(
      (err) => `${err.instancePath || '(root)'} ${err.message}`,
    );
    return { ok: false, errors: errors.length > 0 ? errors : ['schema-invalid'] };
  }
  ...
  if (!schema) {
    return {
      ok: false,
      errors: [`cannot read ${path.basename(SCHEMA_PATH[kind])} — unable to validate this packet`],
    };
  }
  const errors = checkAgainstFloor(obj, schema);
