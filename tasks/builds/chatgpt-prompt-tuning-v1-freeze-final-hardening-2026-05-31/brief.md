...
4. Do NOT bump the OpenAI envelope's `prompt_version` (`openai-spec-review.v2`). Additive Hunt-Target additions do not change the output schema or the reviewer contract; the `.v2` → `.v3` boundary is reserved for breaking changes to the output envelope. DO bump the framework's semver (`.claude/FRAMEWORK_VERSION` + `manifest.json frameworkVersion`) by one patch level — `sync.js:1352` short-circuits when `state.frameworkVersion === frameworkVersion`, so consumers running `/claudeupdate` would report "already on latest" and never propagate the new prompt without a version bump. Add a `.claude/CHANGELOG.md` entry naming the bump and citing this brief. The two version axes are independent: OpenAI envelope version tracks the reviewer contract; framework semver tracks framework-managed file content. See §8 Decision 5.
5. Add a CHANGELOG entry to `.claude/CHANGELOG.md` under `## <new framework version> — <date>` covering the additive Hunt Targets and noting that this is a prompt/review-framework propagation change (not a schema/envelope contract change). Cite this brief.
...
- **Decision 5 (Revision 2 → corrected Revision 5):** Two version axes, both addressed independently. (a) Do NOT bump the OpenAI envelope's `prompt_version` (`openai-spec-review.v2`) on additive Hunt-Target changes — that string tracks the reviewer contract; additive detection patterns do not break it. (b) DO bump the framework's semver by one patch level on any framework-managed file change. `sync.js:1352` short-circuits when `state.frameworkVersion === frameworkVersion`, so without a framework-version bump, consumers running `/claudeupdate` would not propagate the new prompt. The original Decision 5 (Revision 2) conflated the two axes — the precedent cited (2026-05-29 brief) is silent on the framework-version question and PR #9's framework-version stance is ambiguous. This brief explicitly fixes the conflation: add the framework semver bump + `.claude/CHANGELOG.md` entry. External-reviewer / Codex P1 flagged the missing bump as the only blocker before APPROVED. **Inverse trigger (Revision 3):** the `prompt_version` MUST be bumped when any of (a) the output JSON envelope schema changes shape (new required fields, removed fields, retyped fields); (b) a Hunt Target is removed or its detection contract is materially weakened in a way that could cause callers depending on the prior pattern to silently miss findings; (c) the Process passes (currently 1–7 plus second-order integrity pass) are renumbered, removed, or fundamentally re-scoped; (d) the recommendation enum, severity enum, or scope-signal enum changes values. Additive Hunt Targets and clarifying re-wordings do not qualify.
